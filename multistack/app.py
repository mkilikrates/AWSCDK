...
GatewayStack = mygw(app, "MY-GATEWAY", env=myenv, gwtype = 'tgw', gwid = '', route = route, vpc = VPCStack.vpc, bastionsg = BationStack.bastionsg)
S2SVPNStack = s2svpn(app, "MY-VPN", env=myenv, gwtype = 'tgw', route = route, gwid = GatewayStack.gw, cgwaddr = EIPStack.mycustomresource)
S3VPNStack = vpns3(app, "MY-S2SVPNS3", env=myenv, route = route, vpnid = S2SVPNStack.vpn, res = 's3bucket', vpc = VPCStack.vpc)
#ASGStack = asg(app, "MY-ASG", env=myenv, res = 'apachehttp2dmz', preflst = True, allowsg = BationStack.bastionsg, allowall = '', vpc = VPCStack.vpc)
