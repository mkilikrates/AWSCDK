...
InstanceStack = instance(app, "My-linux", env=myenv, res = 'bastion', preflst = True, allowsg = '', grantsg = [EFSStack.efssg.security_group_id], instpol = '', userdata = usrdata, eipall = '', allowall = False, ipstack = ipstack, vpc = VPCStack.vpc, ds = '')
InstanceStack.add_dependency(target=EFSStack)
ECacheStack = ecache(app, "wpmemcache", env=myenv, res = 'memcachet3micromultiaz', preflst = False, domain = VPCStack.hz, allowsg = [InstanceStack.ec2sg], allowall = False, ipstack = ipstack, vpc = VPCStack.vpc)
ECacheStack.add_dependency(target=VPCStack)
OSearchStack = search(app, "wpsearchdomain", env=myenv, res = 'opensearch', preflst = False, domain = VPCStack.hz, allowsg = [InstanceStack.ec2sg], allowall = False, maxaz = 3, ipstack = ipstack, vpc = VPCStack.vpc)
...
ELBStack = alb(app, "WORDPRESSPUB", env=myenv, res = 'wpfe', preflst = False, tgrtip = '', allowsg = '', allowall = [80,443], ipstack = ipstack, tgrt = '', domain = '', certif = [] ,vpc = VPCStack.vpc)
ELBStack.add_dependency(target=CERTStack)
ELBStack2 = alb(app, "WORDPRESSADMIN", env=myenv, res = 'wpbe', preflst = False, tgrtip = '', allowsg = '', allowall = [80,443], ipstack = ipstack, tgrt = '', certif = [], domain = VPCStack.hz, vpc = VPCStack.vpc)
ECStack = ecs(app, "WORDPRESS", env=myenv, res = 'wpecsfar', preflst = False, allowsg = [InstanceStack.ec2sg, ELBStack.lbsg, ELBStack2.lbsg], contenv = [RDSStack.rdsclusterfqdn.domain_name, ECacheStack.clusterfqdn.domain_name], contsecr = [RDSStack.rds.secret], allowall = False, grantsg = [RDSStack.rdssg.security_group_id, EFSStack.efssg.security_group_id, ECacheStack.elasticachesg.security_group_id, OSearchStack.opensearchsg.security_group_id], ipstack = ipstack, srvdisc = '', asg = '', volume = [EFSStack.filesystem.file_system_id], volaccesspoint = [EFSStack.filesystemaccesspoint.access_point_id], lb = [ELBStack.elb, ELBStack2.elb], certif = [CERTStack.cert.certificate_arn], vpc = VPCStack.vpc)
