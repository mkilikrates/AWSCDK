...
S3VPNStack = vpns3(app, "MY-S2SVPNS3", env=myenv, route = route, vpnid = S2SVPNStack.mycustomvpn.ref, res = 's3bucket', vpc = VPCStack.vpc).add_dependency(S2SVPNStack)
S2SVPNStack2 = s2svpn(app, "MY-VPN2", env=myenv, gwtype = gwtype, route = route, res = '', funct = '', ipfamily = 'ipv4', gwid = GatewayStack.gw, cgwaddr = EIPStack2.mycustomresource)
S3VPNStack2 = vpns3(app, "MY-S2SVPNS32", env=myenv, route = route, vpnid = S2SVPNStack2.vpn.ref, res = 's3bucket', vpc = VPCStack.vpc).add_dependency(S2SVPNStack2)
