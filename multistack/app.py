...
ipstack = 'Ipv4'
app = core.App()
VPCStack = VPC(app, "MY-VPC", env=myenv, res = 'protectedvpc', cidrid = 0, natgw = 0, maxaz = 3, ipstack = ipstack)
#FlowLogsStack = flowlogs(app, "MY-VPCFLOW", env=myenv, vpc = VPCStack.vpc)
NetFWStack = netfw(app, "MYNETFW", env=myenv, vpcname = 'protectedvpc', res = 'netfwsinglevpc', vpc = VPCStack.vpc)
...
#ASGStack = asg(app, "MY-ASG", env=myenv, res = 'bastionprotect', preflst = True, allowall = '', ipstack = ipstack, allowsg = '', vpc = VPCStack.vpc).add_dependency(NetFWStack)
...
BationStack = bastion(app, "MY-BASTION", env=myenv, res = 'bastionprotect', preflst = True, allowsg = '', allowall = '', ipstack = ipstack, vpc = VPCStack.vpc).add_dependency(NetFWStack)
...
#EKStack = eks(app, "myeks", env=myenv, res = 'myekspubec2', preflst = True, allowsg = ASGStack.asgsg, allowall = '', ipstack = ipstack, role = '', vpc = VPCStack.vpc)
