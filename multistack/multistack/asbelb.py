...
class asgalbpublicv6(core.Stack):
    def __init__(self, scope: core.Construct, construct_id: str, vpc = ec2.Vpc, bastionsg = ec2.SecurityGroup, **kwargs) -> None:
        ...
        #create listener
        self.elblistnr = self.alb.add_listener(
            "Listener_http",
            port=80,
            protocol=elb.ApplicationProtocol.HTTP,
        )
        # allow access
        self.elblistnr.connections.allow_from(
            self.alb,
            ec2.Port.tcp(80)
        )
        self.elblistnr.connections.allow_default_port_from(
            other=ec2.Peer.any_ipv6(),
            description="Allow from anyone on port 80"
        )
        # associate listener to asg
        self.tgrp = self.elblistnr.add_targets(
            "MyFleet from myasg",
            port=80,
            targets=[self.asg]
        )
