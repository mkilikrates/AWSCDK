...
class VPCEStack(core.Stack):
    def __init__(self, scope: core.Construct, construct_id: str, res, preflst, allowall, ipstack, vpcsrvpolice, vpcsrvtype, vpcsrvprivdomain, vpcsrvsubgrp, vpcsrvport, vpcsrvname = ec2.VpcEndpointService.vpc_endpoint_service_name, vpc = ec2.Vpc, allowsg = ec2.SecurityGroup, vpcstack = core.CfnStack.__name__, **kwargs) -> None:
        ...
        counter = 0
        if vpcsrvprivdomain == '':
            vpcsrvprivdomain = False
        if vpcsrvname != '':
            mypol = ''
            if vpcsrvpolice != '':
                mypol = []
                for police in vpcsrvpolice:
                    newpol = {}
            ...
            if vpcsrvtype == 'Gateway':
                ...
                if mypol != '':
                    for police in mypol:
                        mys3gw.add_to_policy(statement=police)
                        #mys3gw.add_to_principal_policy(statement=police)
            if vpcsrvtype == 'Interface':
                ...
                ressubgrp = vpcsrvsubgrp
                if mypol == '':
                    self.vpcesrv = ec2.InterfaceVpcEndpoint(
                        self,
                        f"{construct_id}",
                        vpc=self.vpc,
                        service=ec2.InterfaceVpcEndpointService(name=vpcsrvname, port=443),
                        subnets=ec2.SubnetSelection(subnet_group_name=vpcsrvsubgrp,one_per_az=True),
                        security_groups=[self.vpcesg],
                        private_dns_enabled=vpcsrvprivdomain
                    )
                else:
                    self.vpcesrv = ec2.InterfaceVpcEndpoint(
                        self,
                        f"{construct_id}",
                        vpc=self.vpc,
                        service=ec2.InterfaceVpcEndpointService(name=vpcsrvname, port=vpcsrvport),
                        subnets=ec2.SubnetSelection(subnet_group_name=vpcsrvsubgrp,one_per_az=True),
                        security_groups=[self.vpcesg],
                        private_dns_enabled=vpcsrvprivdomain
